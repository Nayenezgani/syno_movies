...
try {
	...
	$dbRs = $db->query("SELECT path FROM video_file WHERE mapper_id = {$image_id}");
	$dbRow = $dbRs->fetch(PDO::FETCH_NAMED);
	$path = $dbRow['path'];
	$file_name = dirname($path) . '/@eaDir/' . basename($path) . '/SYNOVIDEO_VIDEO_SCREENSHOT.jpg';

	$image = file_get_contents($file_name);

	header("Content-type: image/jpeg");

	if ($GDsupported && $maxThumbWidth) {
